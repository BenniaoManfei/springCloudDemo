...
public class MovieController {
	...
	public User findUser2(@PathVariable Long id) {
		InstanceInfo instance = eurekaClient.getNextServerFromEureka("MS-PROVIDER-USER-EUREKA-PEER", false);
		if (instance != null) {
	...
	public User findUser2(@PathVariable Long id) {
		...
		User user = restTemplate.getForObject("http://ms-provider-user-eureka-peer/simple/user/"+id, User.class);
	...
	public ServiceInstance findUser3(@PathVariable Long id) {
		ServiceInstance instance = loadBalancerClient.choose("ms-provider-user-eureka-peer");
